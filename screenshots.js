...
(function() {
  function drawNitroRace(ctx, w, h) {
    ...
    // Mountains / Neon city skyline
    ctx.fillStyle = '#1e1b4b';
    ctx.beginPath(); ctx.moveTo(0, h * 0.45);
    ctx.lineTo(w * 0.2, h * 0.3); ctx.lineTo(w * 0.45, h * 0.42);
    ctx.lineTo(w * 0.7, h * 0.25); ctx.lineTo(w, h * 0.45); ctx.fill();
    // Sun
    ctx.fillStyle = '#f43f5e';
    ctx.beginPath(); ctx.arc(w * 0.5, h * 0.38, 22, 0, Math.PI * 2); ctx.fill();
    // Ground & 3D Road perspective
    ...
    ctx.setLineDash([]);
    // Player Car (red speedster)
    ctx.fillStyle = '#ef4444';
    ctx.beginPath(); ctx.roundRect(w * 0.42, h * 0.72, w * 0.16, h * 0.18, 6); ctx.fill();
    ctx.fillStyle = '#0f172a';
    ctx.fillRect(w * 0.45, h * 0.75, w * 0.1, h * 0.08); // Windshield
    ctx.fillStyle = '#38bdf8'; // Headlights
    ctx.fillRect(w * 0.43, h * 0.72, 5, 4); ctx.fillRect(w * 0.55, h * 0.72, 5, 4);
    // HUD overlay
    ctx.fillStyle = 'rgba(0,0,0,0.6)'; ctx.roundRect(10, 10, w - 20, 26, 6); ctx.fill();
    ctx.fillStyle = '#fff'; ctx.font = 'bold 10px sans-serif';
    ctx.fillText('SPEED: 184 KM/H', 16, 26);
  ...
  function drawGamesafe(ctx, w, h) {
    ...
    ctx.fillText('🛡️ GAMESAFE CLOUD', 12, 22);
    ...
    ctx.fillText('🔒 Wipe Fresh Protection: ON', 18, 128);
  }
  ...
  window.screenshots = {
    render(appId, canvas) {
      ...
      if (appId === 'nitrorace') drawNitroRace(ctx, w, h);
