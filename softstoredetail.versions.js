/* FILE: softstoredetail.versions.js — Version history list & multi-version install UI for app details */
(function() {
        ...
        return `
          <div class="store-version-item" style="background:rgba(255,255,255,0.03);border:1px solid var(--border-color);border-radius:10px;padding:10px;margin-bottom:8px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              ...
              <div>
                ${isCurrent ? `
                  <span class="store-ver-installed-badge" style="background:rgba(16,185,129,0.15);color:#10b981;padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700;">
                    Installed
                  </span>` : `
                  <button class="btn-ver-install" data-appid="${appId}" data-ver="${v.version}" data-size="${v.size || 10}" style="background:var(--accent-color);color:#fff;border:none;padding:3px 10px;border-radius:6px;font-size:10px;font-weight:600;cursor:pointer;">
                    Install v${v.version}
                  </button>`}
              </div>
            </div>
            <ul style="list-style:none;margin:0;padding:0;font-size:11px;color:var(--text-muted);">
              ${changesHtml}
            </ul>
          </div>`;
      }).join('');
