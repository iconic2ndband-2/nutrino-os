...
(function() {
  let renderer = null, scene = null, camera = null, animId = null, activeObj = null;

  function cleanup() {
    if (animId) { cancelAnimationFrame(animId); animId = null; }
    if (renderer) { renderer.dispose(); renderer = null; }
    scene = null; camera = null; activeObj = null;
  }
  ...
  function createNebula(s) {
    ...
    const count = 700, geo = new THREE.BufferGeometry(), pos = [], col = [];
  ...
  function createOcean(s) {
    const group = new THREE.Group();
    const planeGeo = new THREE.PlaneGeometry(24, 24, 20, 20);
    const planeMat = new THREE.MeshBasicMaterial({ color: 0x0369a1, wireframe: true, transparent: true, opacity: 0.5 });
    const floor = new THREE.Mesh(planeGeo, planeMat); floor.rotation.x = -Math.PI / 2.3; floor.position.y = -4; group.add(floor);
    const bGeo = new THREE.SphereGeometry(0.15, 8, 8), bMat = new THREE.MeshBasicMaterial({ color: 0x38bdf8 });
    const bubbles = [];
    for (let i = 0; i < 40; i++) {
  ...
  function createNeon(s) {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      const mat = new THREE.MeshBasicMaterial({ color: (i % 2 === 0 ? 0x06b6d4 : 0xec4899), wireframe: true });
      const b = new THREE.Mesh(geo, mat);
      b.position.set((Math.random() - 0.5) * 18, (h / 2) - 5, (Math.random() - 0.5) * 14);
      group.add(b);
    }
    s.add(group);
  ...
  function createVolcano(s) {
    ...
    const count = 120, geo = new THREE.BufferGeometry(), pos = [];
    for (let i = 0; i < count; i++) pos.push((Math.random() - 0.5) * 3, 1.5 + Math.random() * 8, (Math.random() - 0.5) * 3);
    geo.setAttribute('position', new THREE.Float32BufferAttribute(pos, 3));
    const embers = new THREE.Points(geo, new THREE.PointsMaterial({ color: 0xf97316, size: 0.4 }));
    group.add(embers); s.add(group);
    return (t) => {
      cone.rotation.y = t * 0.1;
      const arr = geo.attributes.position.array;
      for (let i = 1; i < arr.length; i += 3) {
        arr[i] += 0.08; arr[i - 1] += Math.sin(t * 2 + i) * 0.02;
        if (arr[i] > 8) { arr[i] = 1.5; arr[i - 1] = (Math.random() - 0.5) * 1.5; arr[i + 1] = (Math.random() - 0.5) * 1.5; }
      }
  ...
  function createAurora(s) {
    const group = new THREE.Group();
    const count = 300, geo = new THREE.BufferGeometry(), pos = [], col = [];
    ...
    geo.setAttribute('position', new THREE.Float32BufferAttribute(pos, 3));
    geo.setAttribute('color', new THREE.Float32BufferAttribute(col, 3));
    const ribbon = new THREE.Points(geo, new THREE.PointsMaterial({ size: 0.45, vertexColors: true, transparent: true, opacity: 0.8 }));
    group.add(ribbon); s.add(group);
    return (t) => {
      const arr = geo.attributes.position.array;
